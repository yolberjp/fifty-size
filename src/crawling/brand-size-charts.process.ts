...
export async function crawlBrand(id: string, params?: {verbose?: boolean}){
    ...
    for(const sizeChart of sizeCharts){
        ...
        const subCategoryIds = canonicalSubCategories.map(canonical=>{
            const subCategory = subCategoriesDB.find(subCategory=>subCategory.canonical === canonical)
            if(!subCategory){
        ...
        // SizeChartCategory
        const sizeChartCategories = [...categoryIds.map((categoryId, index) => ({
                sizeChartId,
...
function init(){
    const argv = yargs(hideBin(process.argv)).argv as { brandId?: string, allbrands?: boolean, updateBrand?: boolean, verbose?: boolean }
